...
productSceneTest.action(/size_(.+)_([^_]+)/, async(ctx) => {
    ...
    ctx.session.selectedSize = { productId, size };
   
    // Edit the button to add a check mark
    const newMarkup = ctx.callbackQuery.message.reply_markup.inline_keyboard.map(row =>
        row.map(button => button.callback_data === ctx.callbackQuery.data ? { ...button, text: `${button.text} ✅` } : button)
      );
