...
const { EtDatetime, ETC, BahireHasab } = require('abushakir');
...
module.exports = {
    sendCalendarMessage: async function (ctx) {
        return await ctx.replyWithHTML("Kindly select your preferred delivery date 🚚", module.exports.calendarButtons(null))
    },
    editMessageByID: async function (ctx, messageID, month) {
        return await ctx.telegram.editMessageText(ctx.chat.id, messageID, undefined, "Kindly select your preferred delivery date 🚚", module.exports.calendarButtons(month - 1))     // Moment months (0-11)
    },
    ...
    getDatesInMonth: function (month, year) {
        ...
        for (var i = firstWeekday; i > 0; i--) {
            ...
            // const udate = moment().date(daysInMonth - i).toDate();
            // console.log("date...........", udate)
            // const date = new Date(udate);
            const now = new EtDatetime(); 
            console.log("now.date;  ...........",now.date) // => {year: 2012, month: 7, day: 28}
            console.log("now.time;  ...........",now.time) 
            const year = now.year
            const month = now.month
            const day = now.day
            console.log("converteddate...........", year, month, day)
            
            const ethiopianDate = new EtDatetime(year, month, day);
            console.log("conethiopiasn date ...........",ethiopianDate.day.toString())
       // => 2012-07-28 17:18:31.466
    ...
    getTodayDate: function () {
        const now =  new EtDatetime(); 
        return module.exports.getSpecificDate(now.year, now.month + 1, now.day)
    },
